...
class WPF_Staging_Sites {
	...
	public function process_actions() {
		...
		if ( ! empty( $_REQUEST['_wpfnonce'] ) && wp_verify_nonce( $_REQUEST['_wpfnonce'], 'wpf_duplicate_site' ) && isset( $_GET['wpf_duplicate_site'] ) ) {
			...
			if ( 'update' === $_GET['wpf_duplicate_site'] ) {
				...
				wpf_log( 'notice', get_current_user_id(), 'Site URL changed from <strong>' . $this->get_site_url() . '</strong> to <strong>' . get_site_url() . '</strong>. Staging mode deactivated.', array( 'source' => 'staging-mode' ) );
	...
	public function is_duplicate_site() {
		...
		$wpf_site_url_parts = wp_parse_url( $this->get_site_url() );
	...
	public function show_staging_notice() {
		...
		if ( $this->is_duplicate_site() && current_user_can( 'manage_options' ) ) {
			...
			printf(
				...
				'<a href="' . esc_url( $this->get_site_url() ) . '" target="_blank">',
				'</a>',
				esc_url( $this->get_site_url() ),
