...
class WPF_GForms_Integration extends GFFeedAddOn {
	...
	public function frontend_conditional_operators() {
		...
		if ( ! wpf_is_user_logged_in() || is_admin() || wp_is_json_request() ) {
			return;
		}
			...
			<script type="text/javascript">
				...
				function set_rule_info( str, objectType, ruleIndex, selectedFieldId, selectedValue ) {
					...
					if ( selectedFieldId === 'wpfusion' ){
						str = `<select data-js-rule-input="value" class="gfield_rule_select gfield_rule_value_dropdown_cl" id="field_rule_value_0" name="field_rule_value_0">`;
				...
				gform.addFilter('gform_conditional_logic_operators', function (operators, objectType, fieldId) {
					if( fieldId === 'wpfusion' ){
						operators.has_tag = '<?php _e( 'user has tag', 'wp-fusion' ); ?>';
						operators.not_has_tag = '<?php _e( 'user does not have tag', 'wp-fusion' ); ?>';
					}
					return operators;
				} );
				...
				gform.addFilter('gform_conditional_logic_operators', function (operators, objectType, fieldId) {
					if(fieldId === 'wpfusion'){
						operators = {
							'has_tag'     : '<?php _e( 'user has tag', 'wp-fusion' ); ?>',
							'not_has_tag' : '<?php _e( 'user does not have tag', 'wp-fusion' ); ?>'
						}
					}
