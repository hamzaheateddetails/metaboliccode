...
class Plugin {
	...
	public function init() {
		...
		$promo_start_date = strtotime( '2022-07-25 00:00' );
		$promo_end_date = strtotime( '2022-08-01 23:59' );
		if ( $now > $promo_start_date && $now < $promo_end_date ) {
			$this->promo_notice_birthday_four();
		} else {
			$this->promo_notice();
		}
	...
	public function promo_notice() {
		$msg = sprintf( __( '%1$sBuy now Dynamic.ooo - Dynamic Content for Elementor%2$s and save 10&#37; using promo code %3$sVISIBILITY%4$s.', 'dynamic-visibility-for-elementor' ) . '<br />', '<a href="https://www.dynamic.ooo/upgrade/visibility-to-premium?utm_source=wp-plugins&utm_campaign=plugin-uri&utm_medium=wp-dash-promo">', '</a>', '<strong>', '</strong>' );
		$msg .= sprintf( __( 'We give you %1$sover 140 features for Elementor%2$s that will save you time and money on achieving complex results. We support ACF Free and ACF Pro, JetEngine, WooCommerce, WPML, Search and Filter Pro, Pods, and Toolset.', 'dynamic-visibility-for-elementor' ), '<strong>', '</strong>' );
		$this->notices->info( $msg, 'upgrade_10' );
	}

	/**
	 * Promo Notice Birthday Four
	 *
	 * @return void
	 */
	public function promo_notice_birthday_four() {
		$msg = sprintf( __( '%1$sDynamic.ooo - Dynamic Content for Elementor Birthday Sale up to 40&#37; off!%2$s', 'dynamic-visibility-for-elementor' ), '<a href="https://www.dynamic.ooo/promo/dynamic-ooo-turns-four/?utm_source=wp-plugins&utm_campaign=plugin-uri&utm_medium=wp-dash-promo">', '</a>', '<strong>', '</strong>' );
		$this->notices->info( $msg, 'upgrade_birthday_four' );
	}
