...
if ( ! class_exists( 'HubWooAjaxHandler' ) ) {
	...
	class Hubwoo_Ajax_Handler {
		...
		public function __construct() {
			...
			// migration notice and subscribe form 
			add_action( 'wp_ajax_hubwoo_subscriber_list', array( $this, 'hubwoo_subscriber_list' ) );
		}
		...
		public function hubwoo_hide_rev_notice() {
			...
			if ( 'review' ==  $type ) {
				...
				update_option( 'hubwoo_hide_migration_notice', 'yes' );
			}
		 ...
		 */
		public function hubwoo_subscriber_list() {
			...
			if (isset($_POST['email'])) {
				...
				$form_id   = 'a9f59a1d-31a0-499b-808f-c58332f29cdf';
				$portal_id = '5373140';

				if ( ! empty( $data ) ) {
					$data    = array( 'fields' => $data );
					$context = Hubwoo_Admin::hubwoo_page_source();
				...
				if ( 200 == $response['status_code']  ) {
					update_option( 'hubwoo_has_subscribed', 'yes' );
					echo wp_json_encode(
						array(
							'response'   => $response,
							'response' => 'subscribed succesfully',
						)
					);
					wp_die();
				} else {
