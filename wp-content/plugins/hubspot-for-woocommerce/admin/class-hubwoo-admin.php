...
class Hubwoo_Admin {
	...
	public function hubwoo_review_notice() {
		...
		<div class="notice notice-success hubwoo-review-notice-wrapper">
			<div class="hubwoo-ocs-options hubwoo-pl-notice">
				...
				<div class="hubwoo-wrapper-notice">
					...
					<a class="hubwoo-close-size hubwoo-hide-rev-notice" href="#" type="review">Dismiss</a>
				</div>
			</div>
		</div>	
		...
		if ( 'yes' != get_option( 'hubwoo_hide_migration_notice', 'no' )  ) { 
			?>
		...
		<div class="notice notice-success hubwoo-review-notice-wrapper" >
			<div class="hubwoo-ocs-options hubwoo-pl-notice">
				<p>
					<strong><?php echo esc_html__( "Dear user, HubSpot is discontinuing their eComm Bridge API. But don't worry, it'll have no impact on the integration(s) you're using. We're working on this and will release an update soon with latest APIs to comply with HubSpot's latest norms.", 'makewebbetter-hubspot-for-woocommerce' ); ?></strong>
				</p>
				
				<div class="hubwoo-wrapper-notice">
					<a class="hubwoo-close-size hubwoo-hide-rev-notice" href="#" type="migration">Dismiss</a>
				</div>
			</div>
			<?php
			if ( 'yes' != get_option( 'hubwoo_has_subscribed', 'no' ) ) {
				?>
					<div class="subsc_email_div hubwoo-pl-notice">
						<form action="#" id="email_form" method="POST">
							<label for="subsc_email"><strong>Subscribe us to stay updated</strong></label>
						<?php
						$current_user = wp_get_current_user();
						$email = $current_user->user_email;
						?>
							<input type="email" id="email" value="<?php echo esc_attr( $email); ?>">
							<input type="submit" id="submit" class="hubwoo-btn--notific" name="submit" value="Submit">
						</form>
					</div>
					<div class="thanks_msg hubwoo-pl-notice">
						<p>
							<strong><?php echo esc_html__( 'Thanks for subscribing us :)', 'makewebbetter-hubspot-for-woocommerce' ); ?></strong>
						</p>
					</div>
			<?php
				...
				if ( $count <= 5 ) {
					...
					if ( $hubwoo_ecomm_order instanceof WC_Order ) {
						...
						if ( ! empty( $customer_id ) ) {
							...
							$contact_preferred_lang = get_user_meta( $customer_id, 'hubwoo_preferred_language', true );
						} else {
							$contact = get_post_meta( $order_id, '_billing_email', true );
							$invalid_contact = get_post_meta( $order_id, 'hubwoo_invalid_contact', true );
							$contact_preferred_lang = get_post_meta( $order_id, 'hubwoo_preferred_language', true );
						}
					}
					...
					if ( $flag ) { 
						...
						if ( 200 == $response['status_code'] ) {
							...
							if ( 'yes' == get_option( 'hubwoo_deal_multi_currency_enable', 'no' ) ) {
								...
								if ( ! empty( $properties ) && ! empty( $deal_id ) ) {
									$properties = array( 'properties' => $properties );
									$response = HubWooConnectionMananager::get_instance()->update_existing_deal( $deal_id, $properties );
								}
							}
				...
				if ( ! empty( $product_type ) && ( 'variable' == $product_type || 'variable-subscription' == $product_type ) ) {
					...
					if ( is_array( $wc_products_array ) && count( $wc_products_array ) ) {
						foreach ( $wc_products_array as $single_var_product ) {
							...
							do_action( 'hubwoo_update_product_property', $single_var_product->ID );
						}
					}
				} else {
					...
					do_action( 'hubwoo_update_product_property', $post_ID );
				}
			}
		}
		if ( count( $updates ) ) {
